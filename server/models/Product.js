...
class Product {
  constructor(data) {
    ...
    this.sizes = typeof data.sizes === 'string' ? JSON.parse(data.sizes || '[]') : (data.sizes || []);
    ...
    // Специфические атрибуты для каждого типа товара
    this.footwear_attributes = data.footwear_attributes ?
      (typeof data.footwear_attributes === 'string' ? JSON.parse(data.footwear_attributes) : data.footwear_attributes) : null;
    this.clothing_attributes = data.clothing_attributes ?
      (typeof data.clothing_attributes === 'string' ? JSON.parse(data.clothing_attributes) : data.clothing_attributes) : null;
    this.toys_attributes = data.toys_attributes ?
      (typeof data.toys_attributes === 'string' ? JSON.parse(data.toys_attributes) : data.toys_attributes) : null;
    this.accessories_attributes = data.accessories_attributes ?
      (typeof data.accessories_attributes === 'string' ? JSON.parse(data.accessories_attributes) : data.accessories_attributes) : null;
  ...
  static async findAll(filters = {}) {
    ...
    if (filters.product_types && filters.product_types.length > 0) {
      const placeholders = filters.product_types.map(() => '?').join(',');
    ...
    if (filters.colors && filters.colors.length > 0) {
      const colorConditions = filters.colors.map(() => `p.color LIKE ?`).join(' OR ');
    ...
    // Size filter (supports both string and number sizes)
    if (filters.sizes && filters.sizes.length > 0) {
      const sizeConditions = filters.sizes.map(() => `p.sizes LIKE ?`).join(' OR ');
      sql += ` AND (${sizeConditions})`;
  ...
  static async getSizes() {
    const sql = 'SELECT DISTINCT sizes FROM products WHERE sizes IS NOT NULL AND sizes != ""';
    const result = await db.query(sql);
