...
router.get("/", async (req, res) => {
  try {
    const filters = {
      ...
      product_types: req.query.product_types
        ? req.query.product_types.split(",")
        : null,
      ...
      sizes: req.query.sizes
        ? req.query.sizes
            .split(",")
            .map((size) => (isNaN(size) ? size : parseInt(size)))
        : null,
...
router.post("/", async (req, res) => {
  try {
    ...
    console.log(`Creating product with request size: ${requestSize} bytes`);
    ...
    if (error.type === "entity.too.large") {
      return res.status(413).json({
        success: false,
        message:
          "Данные слишком большие. Попробуйте уменьшить размер изображений.",
